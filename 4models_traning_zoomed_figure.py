...
if all_forecasts:
    ...
    for model_name, forecast in all_forecasts.items():
        # Connecting forecast to the last training point
    ...
    plt.title(f'Sea Level Forecasting for {LOCATION_TO_FORECAST} (Zoomed Forecast Region)')
    plt.xlabel('Date')
    ...
    # =========================================================================
    # NEW ZOOM LOGIC: Restrict X-axis to focus on the circled forecast area
    # Zooms in to show exactly 2.5 years before the forecast ends
    # =========================================================================
    zoom_start = test_data.index[-1] - pd.DateOffset(months=30)
    zoom_end = test_data.index[-1] + pd.DateOffset(months=2) # Adds slight padding on the right
    plt.xlim(zoom_start, zoom_end)
    # =========================================================================

    plot_filename = os.path.join(output_dir, f'{LOCATION_TO_FORECAST}_forecast_5_models_ZOOMED_{START_YEAR_FILTER}_{END_YEAR_FILTER}.png')
    plt.savefig(plot_filename, dpi=300, bbox_inches='tight')
    print(f"💾 Zoomed Plot saved to: {plot_filename}")
